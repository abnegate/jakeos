...
10. Every CI matrix entry named in Verification (`qemu-x86_64`, `hw-h002`, and so on) is declared by a hardware entry's `Matrix entry`, and every task in that entry's `Provided by` is done (E-116). The environment a task is verified on must exist before the task is done; it is not a dependency, so the task may start earlier.
...
| H hardware | `registers/hardware.md` | Kind, Tier, CPU, GPU, Network, First milestone, Matrix entry, Provided by, Status | Kind qemu/desktop/laptop; Status planned/procured/in-lab/retired |
...
- **Hardware** entries are QEMU profiles and physical machines. `Tier` is `1`, `2` or `none` (QEMU profiles). `First milestone` is the earliest rung whose gates run on it and that rung's `Hardware scope` lists it; a procurement or bring-up task must exist in an earlier or equal rung. `Matrix entry` is the canonical CI matrix-entry name Verification lines use (`qemu-x86_64`, `hw-h002`); a Verification line naming an entry no hardware declares is a warning (W-019). `Provided by` names the tasks that make the entry real (procurement and racking for a machine, the QEMU matrix or profile task for a profile); an entry with no provider, or a provider in a later rung than `First milestone`, is a warning (W-018), and a task verified on the entry cannot be done before its providers are (7.4 item 10). QEMU profiles satisfy functional and compatibility gates; benchmark reports are required on them only when the B-ID's method names QEMU.
